...
Ownable2StepUpgradeable {
    ...
    function mint(uint256 amount, address to) external payable whenNotPaused {
        require(isMintingLogic[msg.sender], "Caller is not valid");
    ...
    function airdrop(address[] memory to, uint256 tokenAmount) external onlyOwner {
        for (uint i = 0; i < to.length; i++) {
            require(totalSupply() + tokenAmount <= MAX_SUPPLY, "Max supply exceeded");
    ...
    // --Upgraded Logic--
    mapping (address => bool) public isMintingLogic;
    
    function setMintingLogic(address caller, bool status) external onlyOwner {
        isMintingLogic[caller] = status;
    }
